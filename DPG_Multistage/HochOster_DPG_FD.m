%Hochburck-Osterman Equation in 2D+time with DPG multistage method%
mkdir(pwd,'Results_HochOster_DPG_FD')
mypath = fullfile(pwd,'Results_HochOster_DPG_FD');
...
for r = rmax:rmax 
    ...
    times_HEuler = zeros(mmax,1);
    times_DPG2 = zeros(mmax,1);
    times_DPG3 = zeros(mmax,1);
    taum = zeros(mmax,1);
    nsteps = zeros(mmax,1);
    ...
    for m = 1:mmax %tau=T/2^m time step size  
        ...
        for i = 1:steps
            ...
            Gn = Fun-JnB*U(:,i);
            U0int(:,i) = U(:,i)+tau*phiB(tau*JnB,[Fun zeros(dimx+1,1)]);
            U(:,i+1) = U(:,i)+tau*JnB*U0int(:,i)+tau*Gn;
        end
        times_HEuler(m) = toc;
        ...
        for i = 1:steps
            ...
            Fun = [1; 1./(1+U(2:end,i).^2)+flin(:,i)-A*U(2:end,i)];
            Gn = Fun-JnB*U(:,i);
            Un2(:,i) = U(:,i)+tau*phiB(tau*JnB,[Fun zeros(dimx+1,1)]);
            Fun2 = [1; 1./(1+Un2(2:end,i).^2)+flin12(:,i)-A*Un2(2:end,i)];
            Gn2 = Fun2-JnB*Un2(:,i);
            dGn2=8*Gn2-8*Gn;
            U(:,i+1) = U(:,i)+tau*phiB(tau*JnB,[8*Gn2-8*Gn zeros(dimx+1,1) Fun]);
        end
        times_DPG2(m) = toc;
        ...
        for i = 1:steps
            ...
            Gn = Fun-JnB*U(:,i);
            Un2(:,i) = U(:,i)+tau*phiB(tau*JnB,[Fun zeros(dimx+1,1)]);
            Un3(:,i) = U(:,i)+tau*JnB*Un2(:,i)+tau*Gn;
            %Evaluations
            Fun2 = [1; 1./(1+Un2(2:end,i).^2)+flin12(:,i)-A*Un2(2:end,i)];
            Gn2 = Fun2-JnB*Un2(:,i);
            ...
            Gn3 = Fun3-JnB*Un3(:,i);
            %Correction factor
            Jn2 = spdiags(-2*Un2(2:end,i)./(1+Un2(2:end,i).^2).^2,0,dimx,dimx)-A;
            dGn2=[0 zeros(1,dimx); vn2(:,i) Jn2]-JnB;
            Cn2=-(1/4)*dGn2*(Un3(:,i)-2*Un2(:,i)+U(:,i));
            %Update
            U(:,i+1) = U(:,i)+tau*phiB(tau*JnB,[-48*Gn2+12*Gn3+36*Gn-48*Cn2 16*Gn2-2*Gn3-14*Gn+16*Cn2 zeros(dimx+1,1) Fun]);
        end
        times_DPG3(m) = toc;
        ...
        Tab_times = table(nsteps,times_HEuler,times_DPG2,times_DPG3,'VariableNames',{'Nsteps','Times_HEuler','Times_DPG2','Times_DPG3'});
        writetable(Tab_times,fullfile(mypath,['TimesHochOster_DPG_r' num2str(r) '_m' num2str(m) '.txt']),'Delimiter',' ');
        
        Tab_errors = table(taum,Errors1,Errors2,Errors3,'VariableNames',{'tau','Err_HEuler','Err_DPG2','Err_DPG3'});
        writetable(Tab_errors,fullfile(mypath,['ErrHochOster_DPG_r' num2str(r) '_m' num2str(m) '.txt']),'Delimiter',' ');
